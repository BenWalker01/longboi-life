...
public class InputManagerTests extends AbstractHeadlessGdxTest {
    ...
    public void testCameraMovesUp() {
        ...
        float expectedY = previousY + (gameState.cameraSpeed * Gdx.graphics.getDeltaTime() * MainCamera.camera().zoom
    ...
    public void testCameraMovesLeft() {
        ...
        float expectedX = previousX - (gameState.cameraSpeed * Gdx.graphics.getDeltaTime() * MainCamera.camera().zoom
                * gameState.scaleFactor);
    ...
    public void testNoCameraMovementWithOpposingInputsHorizontal() {
        ...
        inputManager.handleContinuousInput();
        assertAll(
                () -> assertEquals(world.getWidth() * Constants.TILE_SIZE * gameState.scaleFactor / 2,
                        MainCamera.camera().position.x, 0.01f, "Camera X position should not change"),
                () -> assertEquals(world.getHeight() * Constants.TILE_SIZE * gameState.scaleFactor / 2,
                        MainCamera.camera().position.y, 0.01f, "Camera Y position should not change"),
    ...
    public void testClampingForExtremeNegativeValueForScrolling() {
        ...
        MainCamera.camera().update();
        assertEquals(Constants.MIN_ZOOM, MainCamera.camera().zoom, 0.01f,
                "Camera zoom should clamp to Constants.MIN_ZOOM");
    }
    ...
    public void testClampingForExtremePositiveValueForScrolling() {
        ...
        MainCamera.camera().update();
        assertEquals(Constants.MAX_ZOOM, MainCamera.camera().zoom, 0.01f,
                "Camera zoom should clamp to Constants.MAX_ZOOM");
    }
    ...
    public void testCorrectionForNaNYValue() {
        ...
        MainCamera.camera().update();
        assertEquals(Constants.MAX_ZOOM, MainCamera.camera().zoom, 0.01f,
                "Camera zoom should not change for invalid floating point values");
    }
